...
class PomodoroGameLauncher(QWidget):
    def __init__(self):
        ...
        self.enable_fade_on_break_end = True  # 残り1分で透明化するかどうか
    ...
    def initUI(self):
        self.setWindowTitle('ブレイクゲート ― 作業・休憩・モード選択')
        self.resize(600, 460)

        layout = QVBoxLayout()

        # タイトル
        title_label = QLabel('ブレイクゲート', self)
        title_label.setAlignment(Qt.AlignCenter)
        title_label.setStyleSheet("font-size: 36px; font-weight: bold; padding: 8px;")
        layout.addWidget(title_label)

        self.start_button = QPushButton('作業・休憩時間とモード選択', self)
        self.start_button.setStyleSheet("font-size: 28px; padding: 12px;")
        ...
        guide_label = QLabel(
            "■ あそびかた\n"
            "  1. 上のボタンから作業/休憩時間と休憩中のモードを設定\n"
            "  2. 作業タイマー終了後、右上の［休憩を開始する］を押す\n"
            "  3. 選んだモード（ミニゲーム等）が全画面で起動\n"
            "  4. 休憩終了が近づくと画面が透過し、作業へ戻れます\n"
            "\n"
            "■ 操作キー\n"
            "  ・音楽ゲーム : D / F / J / K（4レーン）\n"
            "  ・テトリス   : ←→ 移動 / A・S 回転 / ↑ ハード / ↓ ソフト\n"
            "\n"
            "■ 注意\n"
            "  「スクリプト実行 / アプリ・EXE実行」モードでは、中断できない\n"
            "  処理の実行中に［休憩終了］を押さないでください。",
            self
        )
        guide_label.setStyleSheet(
            "font-size: 14px; padding: 12px; background: #f4f7fb;"
            "border: 1px solid #c8d2e0; border-radius: 6px;"
        )
    ...
    def setup_session(self):
        ...
        # ★追加：残り1分で透明化するかどうかを選択
        fade_choice, ok_fade = self._big_get_item(
            "透明化設定", "休憩終了1分前にウィンドウを透明にしますか？",
            ["透明にする", "透明にしない"],
            0, False
        )
        if not ok_fade: self._cancel_to_home(); return
        self.enable_fade_on_break_end = (fade_choice == "透明にする")
    ...
    def start_white_only(self):
        ...
        self.timer_win = TimerWindow(
            ...
            screen=scr,
            one_minute_cb=self._start_return_to_work_fade if self.enable_fade_on_break_end else None  # ※enable_fade_on_break_end で制御
        )
        self.timer_win.show()
        QTimer.singleShot(0, lambda: bring_front_noactivate(self.timer_win))
    ...
    def start_break_timer(self):
        ...
        self.on_break_end,
        screen=scr,
        one_minute_cb=self._start_return_to_work_fade if self.enable_fade_on_break_end else None)
        self.timer_win.show()
        QTimer.singleShot(0, lambda: bring_front_noactivate(self.timer_win))
        QTimer.singleShot(80, lambda: bring_front_noactivate(self.timer_win))
    ...
    def on_break_end(self):
        ...
        try:
            # ゲーム以外の休憩（ホワイトアウトなど）の場合はここで止めたりするが
            # ゲームの場合は「遊び続けたい」要望があるので、STOPせずにボタンだけ出す
            pass
        except Exception:
            pass

        # まずBreakボタン（開始用）があれば閉じる
        if hasattr(self, 'break_button_win') and self.break_button_win:
        ...
        # ★ 追加：音楽ゲーム / テトリス の場合も「休憩終了」ボタンを出し、自動終了しない
        current_mode = getattr(self, "mode", "")
        if current_mode in ("音楽ゲーム", "テトリス"):
            # ここでは game_window / tetris_window を閉じない
            # 一応プレビュー白抜きなどは解除しておくと見やすいかも（好みによる）
            try:
                self.preview.stop_whiteout_others()
            except Exception:
                pass
            ...
            self.finish_break_win = FinishBreakWindow(
                "休憩終了（ゲームを閉じる）",
                on_confirm=lambda: self._confirm_stop_runner("game"),
                screen=scr,
                parent=self
            )
            self.finish_break_win.show()
            # ゲームウィンドウたちを最前面解除したり、フォーカスを維持したりする
            # 現状はそのまま（FinishWindowがさらに上に来るはず）
            return

        # それ以外の謎モードや通常時はこれまで通り即終了
        self._close_game_windows()
        self._prompt_next_session()

    def stop_game(self):
        """休憩終了ボタンで呼ばれる：ゲームウィンドウを閉じる"""
        self._close_game_windows()
        
        # 完了通知
        notice = QLabel("休憩終了！ゲームを閉じました。")
        notice.setWindowFlags(Qt.WindowStaysOnTopHint)
        notice.setAlignment(Qt.AlignCenter)
        notice.setStyleSheet("font-size:18px;padding:10px;")
        notice.show()
        def finish_notice():
            ...
            self.restart_cycle()
        QTimer.singleShot(1500, finish_notice)
